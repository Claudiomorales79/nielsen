...
const MAX_AUDITS = 50;
const TMP_FILE = "/tmp/nielsen-audits.json";
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === "POST" && pathname === "/api/login") {
      ...
      runtimeState.sessions.set(token, { createdAt: Date.now() });
    ...
    if (req.method === "PUT" && /^\/api\/audits\/[^/]+$/.test(pathname)) {
      const body = await readJsonBody(req);
    ...
    if (req.method === "DELETE" && /^\/api\/audits\/[^/]+$/.test(pathname)) {
      const id = pathname.split("/").pop();
      const audits = await readAudits();
    ...
    if (req.method === "POST" && pathname === "/api/analyze") {
      ...
      const nextAudits = [nextAudit, ...audits.filter((item) => item.id !== nextAudit.id)].slice(
        0,
        MAX_AUDITS
      );
      await writeAudits(nextAudits);
    ...
    if (req.method === "GET" && /^\/api\/audits\/[^/]+\/report$/.test(pathname)) {
...
function isAuthenticated(req) {
  ...
  return runtimeState.sessions.has(cookies[SESSION_COOKIE]);
}

function setSessionCookie(res, token) {
  res.setHeader(
    "Set-Cookie",
    `${SESSION_COOKIE}=${token}; HttpOnly; Path=/; SameSite=Lax; Max-Age=28800; Secure`
  );
}
...
async function readAudits() {
  if (Array.isArray(runtimeState.audits)) {
    return runtimeState.audits;
  }

  try {
    const raw = await fs.readFile(TMP_FILE, "utf8");
    const parsed = JSON.parse(raw);
    runtimeState.audits = Array.isArray(parsed) ? parsed : [];
  } catch {
    runtimeState.audits = [];
  }
...
async function writeAudits(audits) {
  runtimeState.audits = audits;
  try {
    await fs.writeFile(TMP_FILE, JSON.stringify(audits, null, 2), "utf8");
  } catch (error) {
    console.warn("No se pudo escribir en /tmp:", error.message);
  }
}
...
function normalizeExternalResults(payload) {
  ...
  return items.map((item, index) => {
    ...
    return {
      ...
      name: heuristics[index],
      score,
      severity: ["good", "warn", "bad"].includes(item.severity)
        ? item.severity
        : getSeverity(score),
...
function buildSuggestion(name, severity, mode) {
  ...
  return `${severityCopy[severity]} en "${name}", ${heuristicHints[heuristics.indexOf(
    name
  )]}, ${modeCopy}`;
}
...
function renderReport(audit) {
  const overall = Math.round(
    audit.heuristics.reduce((sum, item) => sum + item.score, 0) /
      Math.max(audit.heuristics.length, 1)
  );
      ...
      <div class="report">
        ...
        <section class="summary">
          ...
          <div>
            ...
            <p>El informe combina el análisis automático del backend con observaciones editables del equipo UX.</p>
          </div>
        </section>
        <section class="cards">
          ${audit.heuristics
